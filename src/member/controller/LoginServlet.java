...
// @WebServlet() 어노테이션
// web.xml에 별도의 url mapping 작업을 하지않고
...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser != null) { // 로그인 성공
			...
			// Session : JSP 내장 객체
			// 웹 브라이저에서 정보를 유지하기 위한 목적으로 주로 사용
			// 일정 시간동안 정보를 유지 시키고 만료시키는 것이 가능.
			...
			// 세션에 정보 입력 시 브라우저가 꺼질 때까지 유지됨
			// 별도의 request에 담아 넘겨줄 필요 없음.
			// -> forward() 필요없음
			
			response.sendRedirect(request.getContextPath());
			// 응답 페이지로 이동 후
			// request / response 객체를 새로 생성함.
			// -> 기존 정보 유지 X
			}
